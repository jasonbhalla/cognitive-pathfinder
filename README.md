...
A multimodal routing engine providing a personalized pathfinding algorithm based on subjective, user-specific costs. It fuses static data with real-time transit and traffic APIs to build a dynamic, weighted graph. The system finds the optimal route that minimizes a user's perceived frustration rather than just the raw travel time. Intersections are stored as vertices and edges between intersections correspond to some path between them via some mode of transportation. Based on the user's preferences, edge weights are adjusted and dynamic calculations for the total user cost are performed. The UI provides both an interface to use the program, and also a visualization of the graph and the mathematical breakdown of the performance.

For example, some people dislike transit, but if going to downtown specifically, they would rather take transit compared to driving because they really hate finding parking downtown. Others really despise transit wait times and delays, so they would rather walk for longer compared to transit for shorter (to an extent). Alternatively, some people would rather take transit even if it takes twice as long as walking, but only if the walk contains a hill. The goal of this project is to capture the user's preferences and use that to adjust the weights of the edges, which feeds into the shortest-path algorithm. Ultimately, the user can enter the starting and ending location, and the program will ideally generate the path that is best for THEM specifically.
